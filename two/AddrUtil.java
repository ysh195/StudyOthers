...
public final class AddrUtil {
  ...
  public static List<InetSocketAddress> getAddresses(String s) { // 그리고 메서드만 열어 놨네? 생성자를 막아놔서 익명 객체로 메서드만 실행하는 것도 안 될 텐데?
                                                                // 혹시 이건 상속용으로 쓰려는 건가? 다른 클래스에서 이걸 상속해서 쓰려고?
    if (s == null) { // 입력값이 없으면
  ...
  public static InetSocketAddress getAddress(String s) {
    ...
    // 그냥 이런 절차를 거쳐서 만들어진 InetSocketAddress 하나를 원하는 거였다면 차라리 InetSocketAddress 클래스를 상속해서 커스터마이징 하거나 오버라이드해서 써도 되는 거 아닌가?
    // 이거 내부에서 이런 식으로 ArrayList에 넣었다 빼는 과정은 불필요해 보이는데
  }
}
